...
def _ensure_uv(index_url: str) -> bool:
    if shutil.which("uvx"):
        print("鉁?uv check: uvx found")
        return True

    print("鉁?uv check: uvx not found; installing...")
    env = os.environ.copy()
    ...
    try:
        ...
        print(f"鉁?uv check: installation failed ({stderr})")
        return False
...
def _register_marketplace(url: str, name: str | None) -> bool:
    ...
    if shutil.which("codex"):
        try:
            ...
            print(f"鉁?marketplace: registration failed ({exc})")
            return False
        ...
        if result.returncode == 0 or "already added" in output.lower():
            if "already added" in output.lower():
                ...
                print(f"鉁?marketplace: '{mp_name}' registered via codex CLI")
            return True

        print(f"鉁?marketplace: registration failed ({output.strip()})")
        return False
    ...
    if section_header in existing:
        ...
        print(f"鉁?marketplace: '{mp_name}' written to {config_path}")
...
def _cmd_bootstrap(args: argparse.Namespace) -> int:
    ...
    if doctor_code == 0:
        print("鉁?doctor: checks passed")
    else:
        print("鉁?doctor: checks reported problems")
        ok = False
